/**
  Program activates soft device and drives WS2812 leds.
  Radio timeslots are used to ensure LED update isn't interrupted.
*/
...
#define NRF_LOG_MODULE_NAME "XMASJMPR"
#include "nrf_log.h"
...
#define DEVICE_NAME                     "Pete's Xmas Jumper"                           /**< Name of device. Will be included in the advertising data. */
...
#define BLE_UUID_CUSTOM_CHAR            0x0001
...
APP_TIMER_DEF(m_led_value_change_timer_id);
...
static void timer_led_value_change_handler(void * p_context)
{
    led_pattern_step();

    uint32_t               err_code;

    // Request next radio timeslot event
...
static void services_init(void)
{
    ...
    BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_UUID_CUSTOM_CHAR);
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTS_EVT_WRITE:
            ...
            uint8_t i=0;
            for (i=0;i<p_ble_evt->evt.gatts_evt.params.write.len;i++) NRF_LOG_INFO("Data %d: 0x%x",i,p_ble_evt->evt.gatts_evt.params.write.data[i]);

            if (p_ble_evt->evt.gatts_evt.params.write.handle == char_handle.value_handle)
            {
                uint8_t segment = p_ble_evt->evt.gatts_evt.params.write.data[0];
                uint8_t command = p_ble_evt->evt.gatts_evt.params.write.data[1];
                ...
                led_pattern_command_entry(segment, command, data);
            }
...
uint32_t request_next_event_earliest(void)
{
    ...
    m_timeslot_request.params.earliest.timeout_us  = 50000;
...
nrf_radio_signal_callback_return_param_t * radio_callback(uint8_t signal_type)
{
    switch(signal_type)
    {
        case NRF_RADIO_CALLBACK_SIGNAL_TYPE_START:
            nrf_gpio_pin_toggle(22); //Toggle LED4
...
{   
    ...
    // Create timer
    err_code = app_timer_create(&m_led_value_change_timer_id,
                                APP_TIMER_MODE_REPEATED,
                                timer_led_value_change_handler);
    APP_ERROR_CHECK(err_code);

    err_code = app_timer_start(m_led_value_change_timer_id, APP_TIMER_TICKS(100, APP_TIMER_PRESCALER), NULL);
    APP_ERROR_CHECK(err_code);
}
...
int main(void)
{
    ...
    NRF_LOG_DEBUG("=============\r\n");
    NRF_LOG_DEBUG("Device booted\r\n");
    NRF_LOG_DEBUG("=============\r\n");

    neopixel_init(&m_strip, LED_DATA_PIN, NUM_LEDS);
    led_pattern_init(&m_strip, NUM_LEDS);
